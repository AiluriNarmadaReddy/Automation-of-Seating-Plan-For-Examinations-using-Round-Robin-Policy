...
for i in range(0, hz.no_of_halls):
    sheet_names.append(df.hall_df.iloc[i, 0])
    sheet_names = list(sheet_names)
#print(sheet_names)
year = time.strftime("%y", time.localtime())
mon = time.strftime("%b", time.localtime())
date = time.strftime("%d", time.localtime())
excel_file_name = "out_sheet "+mon + "_" + year + ".xlsx"
print(excel_file_name)
if not os.path.exists(excel_file_name):
    workbook = openpyxl.Workbook()
    default_sheet = workbook['Sheet']
    workbook.remove(default_sheet)
    worksheet = workbook.create_sheet("out_sheet"+mon+"-"+year)
    black = "FF000000"
    yellow = "FFFFFF00"
    ...
    worksheet['A6'].font = Font(size=23, name='Times New Roman')
    worksheet['A6'].alignment = alignment
    worksheet['A6'] = "B.TECH "+str(df.year[0])+"-"+str(df.sem[0])+"  Reg/Supply Room Numbers"
    worksheet['A6'].fill = fill_style
    worksheet['A6'].border = border_style
    ...
    for key, value in ou.merge_dict.items():
        worksheet.merge_cells(start_row=row_num, start_column=1, end_row=row_num + value - 1, end_column=1)
